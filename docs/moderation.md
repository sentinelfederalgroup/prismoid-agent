...
The agent store is intentionally isolated from platform mutation commands. It recommends actions but does not ban, timeout, or delete on its own. A future provider can replace the deterministic analyzer with OpenAI, Ollama, or another model while keeping the same `AgentSnapshot` interface.
